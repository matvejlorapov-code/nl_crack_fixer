    ...
    async def test_lua_library(self, http_base):
        async with aiohttp.ClientSession() as s:
            async with s.get(f"{http_base}/lua/testlib") as r:
                ...
                body = await r.json()
                assert body["name"] == "testlib"
                assert body["code"] == "-- lua library: testlib\n"
                assert body["success"] is True
                assert body["update"] is False

    @pytest.mark.asyncio
    async def test_reqitem_returns_module_object(self, http_base):
        async with aiohttp.ClientSession() as s:
            async with s.get(f"{http_base}/api/reqitem?name=testlib&token=t&cheat=csgo") as r:
                assert r.status == 200
                assert r.headers.get("X-Powered-By") == "Express"
                ct = r.headers.get("Content-Type", "")
                assert "application/json" in ct
